...
%--------------------------------------------------------------------------
if any(infoFile.frameDimensions == 0)
    warning('matlabSTORM:corruptedInfoFile', 'Unexpected frame dimensions');
    infoFile.frameDimensions = [infoFile.hend - infoFile.hstart + 1, ...
        infoFile.vend - infoFile.vstart + 1];
end
end


% Helper function: Coercing field names into a consistent format.
function name = coerceFieldName(name)
% Ignore all characters, extract all words:
words = extract(name, lettersPattern);
for i = 2:length(words)
    words{i}(1) = upper(words{i}(1));
end

% If the first two letters of the first word are uppercase, do nothing; 
% otherwise, force the first letter to lowercase:
if words{1}(1) == upper(words{1}(1)) && words{1}(2) == upper(words{1}(2))
else
    words{1}(1) = lower(words{1}(1));
end
