...
if dataFilesMissing
    ...
    trainDataIdx = randperm(numIAll, min(trainDataSelectNum, numIAll));
    IAll = IAll(:, :, trainDataIdx);
    trainDataSelectNum = numel(trainDataIdx);
...
% Calculate Hamming distance for binary words without pdist2 overhead:
wordsDouble = double(words);
codeDouble = double(codeDic);
hammingDist = sum(wordsDouble, 2) + sum(codeDouble, 2)' - 2 * (wordsDouble * codeDouble');
...
decodedGeneNum = double(values(geneCountDic));
perfectDecodedGeneNum = double(values(perfectGeneCountDic));
